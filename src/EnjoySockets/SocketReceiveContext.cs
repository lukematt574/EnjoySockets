...
{
    internal sealed class SocketReceiveContext : IValueTaskSource<int>
    {
        ...
        internal SocketReceiveContext(ushort maxFullEncryptPacket)
        {
        ...
        /// </summary>
        internal void StartPrepare(int bytesToRead)
        {
        ...
        internal ReadOnlyMemory<byte> GetSaveBytes()
        {
            return _array.AsMemory(0, TotalLength);
        }

        internal ReadOnlySpan<byte> GetSaveBytesSpan()
        {
            return _array.AsSpan(0, TotalLength);
        }

        internal ReadOnlyMemory<byte> GetSaveBytes(EAesGcm aes)
        {
            var lengthData = TotalLength - ETCPSocket.PacketEncryptHeader;
            if (lengthData < 1)
        ...
        internal ReadOnlySpan<byte> GetSaveBytesSpan(EAesGcm aes)
        {
            var lengthData = TotalLength - ETCPSocket.PacketEncryptHeader;
            if (lengthData < 1)
